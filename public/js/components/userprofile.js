...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadData() {
        ...
        const activeUserSession = localStorage.getItem('userData') || sessionStorage.getItem('userData');

        if (activeUserSession) {
            try {
                const sessionData = JSON.parse(activeUserSession);
                const fullUserData = allUsers.find(user => user.email.toLowerCase() === sessionData.email.toLowerCase());
                ...
                if (fullUserData) {
                    ...
                    showMessage('editProfileMsg', "Data pengguna tidak ditemukan.", false);
                }
            } catch (error) {
                console.error('Error parsing user data:', error);
            }
        } else {
            console.error('Tidak ada pengguna yang aktif. Mengarahkan ke halaman login.');
    ...
    function validateForm(newName, newPassword, confirmPassword, newBio) {
        ...
        if (newName && /\d/.test(newName)) {
            showMessage('editProfileMsg', "Nama tidak boleh mengandung angka.", false);
            return false;
        }
        ...
        if (newPassword || confirmPassword) {
            if (newPassword.length < 8) {
                showMessage('editProfileMsg', "Password harus minimal 8 karakter.", false);
                return false;
            }

            if (newPassword !== confirmPassword) {
                showMessage('editProfileMsg', "Password baru dan konfirmasi password tidak cocok.", false);
                return false;
            }
        }
    ...
    function handleProfileUpdate(event) {
        ...
        if (!activeUserData) {
            return showMessage('editProfileMsg', "Gagal menyimpan, user tidak ditemukan.", false);
        }

        // Update data di registeredUsers
        const userIndex = allUsers.findIndex(user => user.email.toLowerCase() === activeUserData.email.toLowerCase());
        ...
        if (userIndex !== -1) {
            ...
            return showMessage('editProfileMsg', "User tidak ditemukan dalam database.", false);
        }

        // Update session data
        const sessionDataString = localStorage.getItem('userData') || sessionStorage.getItem('userData');
